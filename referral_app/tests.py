...
class ReferralAppTests(TestCase):
    ...
    def test_home_view(self):
        """Проверка работы домашней страницы"""
    ...
    def test_create_valid_user_profile(self):
        ...
        response = self.client.post(
            reverse("referral_app:register"), data=data, format="json"
        )
    ...
    def test_update_auth_code_for_existing_user(self):
        ...
        response = self.client.put(
            reverse("referral_app:sign_up", args=[user.pk]), data=data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_retrieve_user_profile(self):
        ...
        user = self.client.post(
            reverse("referral_app:register"), data=data_registration, format="json"
        )

        # Login the user
        data_login = {"phone_number": "1234567890", "auth_code": user.data["auth_code"]}
        response_login = self.client.post(
            reverse("referral_app:login"), data=data_login
        )
        ...
        response = self.client.get(
            reverse("referral_app:retrieve", args=[user.data["id"]])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_retrieve_user_used_invite_code(self):
        ...
        user_1 = self.client.post(
            reverse("referral_app:register"), data=data_registration_1, format="json"
        )

        data_registration_2 = {"phone_number": "9876543210"}
        user_2 = self.client.post(
            reverse("referral_app:register"), data=data_registration_2, format="json"
        )

        # Login the user
        data_login = {
            "phone_number": "1234567890",
            "auth_code": user_1.data["auth_code"],
        }
        response_login = self.client.post(
            reverse("referral_app:login"), data=data_login
        )
        ...
        response = self.client.patch(
            reverse("referral_app:retrieve", args=[user_1.data["id"]]), data=data
        )
